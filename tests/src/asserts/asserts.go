...
func Assert_chan_contains(c <-chan string, expected_value string) {
	log.Println("Waiting for line")
	time.Sleep(1 * time.Millisecond)
	select {
	case line, ok := <-c:
		if ok {
			log.Printf("Got line %s\n", line)
			AssertContains(line, expected_value)
		} else {
			log.Printf("Channel closed")
			panic(1)
		}
...
func AssertContains(value, expected_value string) {
	if !strings.Contains(value, expected_value) {
		log.Printf("Expected value to be contained: '%s'\n", expected_value)
		log.Printf("Got value:      '%s'\n ", value)
		panic(1)
	}
}
