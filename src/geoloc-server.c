...
static inline int parse_msg (struct msg_parts *parts, char *msg, int n) {
   ...
   if (0 == len) { printf("Unable to parse operator"); return -1; }
   parts->operator_len = len;
   parts->taxi = js0n("taxi",0,msg,n,&len);
   if (0 == len) { printf("Unable to parse taxi"); return -1; }
   parts->taxi_len = len;
   parts->lat = js0n("lat",0,msg,n,&len);
   if (0 == len) { printf("Unable to parse lat"); return -1; }
   parts->lat_len = len;
   parts->lon = js0n("lon",0,msg,n,&len);
   if (0 == len) { printf("Unable to parse lon"); return -1; }
   parts->lon_len = len;
   parts->timestamp = js0n("timestamp",0,msg,n,&len);
   if (0 == len) { printf("Unable to parse timestamp"); return -1; }
   parts->timestamp_len = len;
   parts->status = js0n("status",0,msg,n,&len);
   if (0 == len) { printf("Unable to parse status"); return -1; }
   parts->status_len = len;
   parts->device = js0n("device",0,msg,n,&len);
   if (0 == len) { printf("Unable to parse device"); return -1; }
   parts->device_len = len;
   parts->hash = js0n("hash",0,msg,n,&len);
   if (0 == len) { printf("Unable to parse hash"); return -1; }
   parts->hash_len = len;
